...
import { extrapolateAttitude } from "@/lib/attitude-extrapolate";
import {
  drawSkyGround,
  drawPitchLadder,
  drawRollArc,
 ...
 */
// Cap redraw rate — the underlying telemetry itself only updates a few
// times a second over a low-bandwidth radio link, and 30fps is already
// smoother than the eye can tell apart from 60fps for this kind of gauge.
// Halves canvas redraw CPU cost on constrained hardware (e.g. Raspberry Pi).
...
export function OverviewHud() {
  ...
  const draw = useCallback(() => {
    ...
    // Force DPR 1 — HiDPI-scaled canvas buffers cost real CPU per redraw for
    // no perceptible gain on this kind of gauge display.
    const dpr = 1;
    ...
    const { roll, pitch } = extrapolateAttitude(att);
