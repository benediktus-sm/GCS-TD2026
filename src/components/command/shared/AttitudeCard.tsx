...
import { useSmoothedAttitude } from "@/hooks/use-smoothed-attitude";
...
/** SVG Artificial Horizon Indicator. roll/pitch are in degrees. */
...
export function AttitudeCard({ className }: AttitudeCardProps) {
  ...
  const smoothedAttitude = useSmoothedAttitude();

  const { roll, pitch, yaw } = smoothedAttitude;
