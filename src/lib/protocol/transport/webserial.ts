...
export class WebSerialTransport implements Transport {
  ...
  async connect(baudRate: number = 115200): Promise<void> {
    ...
    try {
      ...
      // Explicitly release DTR/RTS (ESP32/CH340-style auto-reset boards hold
      // the MCU in reset if these lines are left in an unknown state on open).
  ...
  private async readLoop(): Promise<void> {
    ...
    try {
      ...
      if (this._connected && !this._disconnecting) {
        this._connected = false;
        // The loop exited abnormally (device error/removal), not via an
        // explicit disconnect() call — release reader/writer locks and close
        // the port ourselves, otherwise the underlying SerialPort stays open
        // forever and every future reconnect attempt fails with
  ...
  /** Release reader/writer locks and close the underlying port, best-effort. */
