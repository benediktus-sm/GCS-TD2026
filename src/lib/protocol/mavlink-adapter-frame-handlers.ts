...
export function requestDataStreams(s: Pick<FrameHandlerState, 'transport' | 'firmwareHandler' | 'targetSysId' | 'targetCompId' | 'sysId' | 'compId'>): void {
  ...
  // Conservative rates — this legacy MAVLink1 request re-fires every 10s
  // (see streamRequestInterval below) and silently overrides whatever the
  // vehicle's own SRx_* params were tuned to. The old hardcoded rates here
  // (EXTRA1 @ 10Hz, POSITION @ 5Hz, EXTRA2 @ 4Hz, ...) summed to >1100 B/s,
  // nearly double a typical LoRa telemetry radio's sustained throughput
  // (~600 B/s), causing an ever-growing buffer backlog and multi-second
  // display latency on low-bandwidth links. Keep these low enough to be
  // safe on any link; fast links (USB/WiFi) don't need a high push rate
  // here anyway since the UI already renders at whatever rate arrives.
  const streams: [number, number][] = [
    [10, 4], [6, 1], [11, 2], [2, 1], [12, 0], [3, 1], [1, 0],
  ]
